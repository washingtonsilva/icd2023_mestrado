...
1. **Simule uma repetição**: modele ou traduza o experimento aleatório usando 
números aleatórios no computador. Uma iteração do experimento é chamada de 
“teste”.

2. **Determine o sucesso**: Com base no resultado da repetição, determine se 
o evento $A$ ocorre ou não. Se sim, chame isso de "*sucesso*".
...
Pela lei dos grandes números, se $(U_i, i\geq 1)$ é uma família de VAs 
uniformemente distribuídas em $[0,1]$, então:
...
## Aplicações: Value-at_Risk (VaR) = Valor-em-Risco (VaR)

- O problema com o desvio padrão como medida de risco é que ele não se 
concentra na cauda negativa da distribuição de lucros/prejuízos. 

- Assim, para dar conta da assimetria, é natural buscar medidas de risco 
relacionadas a *quantis*. 

- A medida mais conhecida é o Valor-em-Risco, vejamos por que as 
características da distribuição empírica dos retornos de 
ativos arriscados implicam na utilização de medidas de 
...
- Tecnicamente, o VaR é um quantil da distribuição de probabilidade de lucros/prejuízos.


## Aplicações: Value-at_Risk (VaR)

**Definição**: *o VaR é a perda em uma carteira tal que existe uma probabilidade* $p$ *de perdas iguais ou superiores ao VaR em um determinado período de negociação e uma probabilidade* $(1-p)$ *de perdas serem inferiores ao VaR. Ou, de forma equivalente, uma probabilidade $p$ de perdas serem superiotes ao VaR*. 

- Podemos escrever $VaR(1-p)$ ou $VaR^(100*(1-p)\%)$ para tornar a dependência 
  do $VaR$ em relação à probabilidade ($p$) explícita. Os níveis mais comuns de 
  probabilidade são 1% e 5% ($VaR(1 - 0.05)$ ou $VaR^{95\%}$).
  
- $VaR$ é um **quantil** da distribuição de lucros e perdas ($P/L$).

```{r var, out.width = "80%", echo = FALSE, fig.align = 'center'}
...
$$
P[Q \leq -VaR(1-p)] = 1 - p = \int_{-\infty}^{-VaR(1-p)} f_q(x)dx
$$
...
\begin{align*}
 p &= P[Q \leq -VaR(1-p)], \\
   &= P(P_t - P_{t-1} \leq -VaR(1-p)), \\
   &= P(P_{t-1} R_t \leq -VaR(1-p)), \\
   &= (\frac{R_t}{\sigma} \leq - \frac{VaR(1 - p)}{P_{t-1}\sigma})
\end{align*}
$$
...
$$
\begin{align*}
VaR(p) &= -\sigma F_{R}^{-1}(1-p) P_{t-1}, \\
       &= -\sigma F_{R}^{-1}(1-p) \vartheta
\end{align*}
$$

## Value-at_Risk (VaR)

- O **nível de confiança** (probabilidade) do VaR, ou seja, 
...
**Definição**: O VaR de h-dias com $100(1-p)\%$ é um número $VaR_{1-p,h}$ tal que a probabilidade de perder $VaR_{1-p,h}$ ou mais, nos próximos h-dias, é igual 
$100(1-p)\%$.
...
diária ($\sigma$ = desvio-padão) é de 2%. Então, o VaR no nível de 5% é:

$$
\begin{align*}
VaR(p) &= \sigma F_{R}^{-1}(p) \vartheta \\
       &= \sigma Z_{0.95} \vartheta \\
       &= 0.02 (1.6449)(100.000) \\
       &= 3289.8
\end{align*}
$$
Isto é, $VaR_{1, 0.95}$ de 1-dia = R\$ 3.289,80, implica que temos 95% de confiança de que a perda da carteira não será superior a R\$ 3.289,80 em um dia. 

Lembrando que podemos calcular o quantil $Z_{0.95} = P(Z \leq z^{\ast}) = 0.95$ em R usando a função: `qnorm(0.95) = 1.644854`

**Exemplo 2**:

Se desejássemos um VaR de um 1 dia com 99% de confiança, teríamos:

$$
\begin{align*}
VaR(p) &= \sigma F_{R}^{-1}(p) \vartheta \\
       &= \sigma Z_{0.99} \vartheta \\
       &= 0.02(2.3263)(100.000) \\
       &= 4652.6
\end{align*}
$$

$VaR_{1, 0.99}$ de 1-dia = R\$ 4.652,60, implica que temos 99% de confiança de 
que a perda da carteira não será superior a R\$ 4.652,60 em um dia.

Claramente, aumentar o nível de confiança em 4% tem um efeito significativo.
...
## Simulação Histórica ou Método Histórico
